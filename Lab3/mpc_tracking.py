...
def main():
    """
    reference = ["sin", "linear", "polynomial"]   ------  reference type
    control_type = ["all", "pos"]   ------  control pos&vel or only pos

    reff = reference[0] # choose the reff type
    control = control_type[0] # choose the control state
    """
    ...
    if reff == "sin":
        ...
        coefficients = [
            [0.05, 0.2, -2.0],  # Joint 1: 0.05*t^2 + 0.2*t - 2.0 (stays within [-2.8973, 2.8973])
            [0.0, 0.05, 0.0],  # Joint 2: -0.05*t^2 + 0.3*t - 1.5 (stays within [-1.7628, 1.7628])
            [0.1, 0.15, -1.5],  # Joint 3: 0.2*t^2 (stays within [-2.8973, 2.8973])
            [0.1, -0.1, 0.1],
            [0.15, 0.2, -2.5],
            [-0.1, 0.4, 0.1],
            [0.05, 0.25, 0.0]
    ...
    for i in range(steps):
        ...
        u_star = tracker.computesolution(x_ref, x0_mpc, u_mpc, H, Ftra, False)
    ...
    for i in range(num_joints):
        ...
        if control=="all":
            plt.plot([qd[i] for qd in qd_d_all], label=f'Desired Velocity - Joint {i+1}', linestyle='--', color='r')
        ...
        # image_dir = "report_figures/different_reference"
        # if not os.path.exists(image_dir):
        #     os.makedirs(image_dir)  # Create the directory if it doesn't exist
        #
        # image_path = os.path.join(image_dir, f"joint_{i + 1}_ref_{reff}_control_{control}_Q_{Q_p}_{Q_v}.png")
